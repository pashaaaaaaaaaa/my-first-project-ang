...
import { SetOne, SetTwo } from './books/table-books/table-book';
import { Hero } from './hero';
import tbook from './input-data.json'
@Injectable({
  providedIn: 'root',
})
export class InMemoryDataService implements InMemoryDbService {
  createDb() {
    ...
    const setOne : SetOne[] = tbook.set2.data.map((a: SetOne) => Object.assign(a, tbook.set1.data.find((b : SetTwo) => b.id == a.id)))
    const setTwo : SetTwo[] = tbook.set1.data 
    return {heroes, };
  }
